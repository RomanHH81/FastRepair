...
interface Review {
  id: string; // Hinzufügen einer ID für besseres Key-Management
  author_name: string;
  rating: number;
  text: string;
}
...
const GoogleReviews = () => {
  ...
  return (
    <ContentSection id="reviews">
      ...
      {reviews.length === 0 ? (
        <p>Aktuell sind keine Bewertungen verfügbar.</p>
      ) : (
        <Flex
          direction={{
        ...
        >
          {reviews.map((review) => (
            <Box
              as="div"
              key={review.id} // Verwendung einer stabileren ID als Key
              style={{
